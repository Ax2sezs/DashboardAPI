     ...
     DateTime? endDate)
        {
            ...
            var lossGrouped = await _context.uvw_El_Calculation
                ...
                .Where(x => string.IsNullOrEmpty(outletNameFilter) || x.OutletName.ToUpper() == outletNameFilter.ToUpper())
                .Where(x => !startDate.HasValue || (x.CalDate.HasValue && x.CalDate.Value.Date >= startDate.Value.Date))
                .Where(x => !endDate.HasValue || (x.CalDate.HasValue && x.CalDate.Value.Date <= endDate.Value.Date))
            ...
            var saleGrouped = await _context.Rpt_EL_CalculateSale
                .Where(s => string.IsNullOrEmpty(outletNameFilter) || s.Branch_Code.ToUpper() == outletNameFilter.ToUpper())
                ...
                .Select(g => new
                {
                    ...
                    Lossell = g.Sum(x => (decimal?)x.LostSalePerBill * (decimal?)(x.TotalBill ?? 0)) ?? 0
                })
                .ToListAsync();
            ...
            // Join
            // =========================
            var result = from l in lossGrouped
            ...
            DateTime? endDate)
        {
            ...
            var query = _context.uvw_El_Calculation.AsNoTracking()
            ...
            var notes = await _context.El_Calculation_Notes
                .AsNoTracking()
                .Where(n => runIds.Contains(n.RunId))
                .ToListAsync();

            var noteCountMap = notes
                .GroupBy(n => n.RunId)
            ...
            var rptSummary = await _context.Rpt_EL_CalculateSale.AsNoTracking()
            ...
            var isSingleDay = startDate.HasValue && endDate.HasValue && startDate.Value.Date == endDate.Value.Date;

            var hourlyDetail = data
                ...
                .Select(g =>
                {
                    ...
                    var summaryForHour = calHourInt >= 0
                    ...
                    return new HourlyTrendDto
                    {
                        ...
                        LostPerBill = summaryForHour != null
                            ? (summaryForHour.LostSalePerBill ?? 0) * (summaryForHour.TotalBill ?? 0)
                            : 0,
                ...
                .OrderBy(x =>
                {
                    if (int.TryParse(new string(x.Hour.TakeWhile(char.IsDigit).ToArray()), out var h))
                        return h;
                    return int.MaxValue;
                })
                .ToList();
        ...
        // Note Feed
        // ============================
        public async Task<List<NoteFeedDto>> GetNotesFeedAsync(string outletNameFilter, DateTime? calDate)
        {
            var query = _context.El_Calculation_Notes
                .Join(_context.uvw_El_Calculation,
                      note => note.RunId,
                      calc => calc.RunId,
        ...
        {
            return await _context.El_Calculation_Notes
                .Join(_context.uvw_El_Calculation,
                      note => note.RunId,
                      calc => calc.RunId,
                      (note, calc) => new { note, calc })
            ...
            DateTime? endDate)
        {
            ...
            var query = _context.uvw_El_Calculation.AsQueryable()
                .Where(x => allowedBranches.Contains(x.OutletName.ToUpper()))
                .Where(x => string.IsNullOrEmpty(outletFilterUpper) || x.OutletName.ToUpper() == outletFilterUpper)
                .Where(x => !startDate.HasValue || (x.CalDate.HasValue && x.CalDate.Value.Date >= startDate.Value.Date))
                .Where(x => !endDate.HasValue || (x.CalDate.HasValue && x.CalDate.Value.Date <= endDate.Value.Date));
